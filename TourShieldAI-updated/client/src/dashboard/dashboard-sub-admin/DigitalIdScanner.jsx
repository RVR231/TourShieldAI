...
const DigitalIdScanner = () => {
  ...
  const processQRResult = (resultData, resolve, reject) => {
    try {
      ...
      if (typeof qrData === 'string' && qrData.includes('TourShieldDigitalID')) {
        verificationData = {
      ...
      reject(new Error('Invalid QR code format. Please scan a valid TourShield Digital ID.'));
    }
  };
  ...
  const testApiConnection = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/digital-id/verify`, {
        ...
        body: JSON.stringify({
          qrData: '{"type":"TourShieldDigitalID","uid":"Xful3qyptCbJbC4S4Udu43FKv8X2"}',
  ...
  const handleFileUpload = async (event) => {
    ...
    try {
      ...
      const requestBody = {
        ...
        hash: qrData.hash,
        type: qrData.type || 'TourShieldDigitalID'
      };
      ...
      let response;
      try {
        response = await fetch(`${import.meta.env.VITE_BASE_URL || 'https://tourshieldai-backend.onrender.com'}/api/digital-id/verify`, {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
            ...
            {showManualInput && (
              <div className="mt-4 p-4 bg-gray-50 rounded-lg">
                ...
                <textarea
                  ...
                  placeholder='{"type":"TourShieldDigitalID","uid":"user_id_here","blockchainId":"blockchain_address"}'

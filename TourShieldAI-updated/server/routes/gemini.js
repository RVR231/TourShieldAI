...
router.post('/chat', async (req, res) => {
  try {
    ...
    const response = await generateTourShieldResponse(message, conversationHistory, location);
    ...
    console.error('TourShield AI Gemini Error:', error);
    res.status(500).json({
      success: false,
...
router.get('/health', (req, res) => {
  res.json({
    success: true,
    service: 'TourShield AI Gemini Service',
    status: 'operational',
    timestamp: new Date().toISOString(),
    version: '1.0.0'
  });
...
// Enhanced response generator for TourShield AI using Gemini AI
async function generateTourShieldResponse(message, conversationHistory = [], location = '') {
  try {
...
Respond as TourShield AI, focusing on travel safety and security.`;
...
function generateGeneralResponse(message, location) {
  return {
    content: `🧭 **TourShield AI Travel Assistant${location ? ` - ${location}` : ''}**
